...
name: etsy-seller
description: >
  Etsy SEO optimization system for existing and new listings. Two core actions auto-detected from input shape — REWRITE when the user pastes a current title + tags + description, CREATE when they provide a short product context. Uses live Etsy autocomplete + competitor SERP scraping for evidence-driven keyword research. Enforces every May 2026 Etsy rule (tag ≤20 chars, primary keyword in first 40 chars, indexing spread across title/tags/attributes/description/hero alt, NLP-aware writing, trademark stoplist). Honestly diagnoses when the problem isn't SEO (CTR / conversion / product / pricing) and gives concrete next-step pointers. Builds and silently maintains a markdown listing database at ~/etsy-listings/ for cross-session memory on Claude/Cowork; session-snapshot pattern for other AI tools. Catches duplicate primary keywords across the user's own listings with a soft warning. Issues a platform-fit reality check when the user enters a saturated niche with a generic product. No shop concept, no cross-shop tracking, no xlsx — every user's listings are treated as one flat database in the active chat thread.
---
